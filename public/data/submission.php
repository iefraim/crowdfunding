...
$query = "INSERT INTO `donations` (`amount`, `first_name`, `last_name`, `shown_name`, `address`, `city`, `state`, `zip`, `phone`, `email`, `comment`, `teamID`, `campaign_id`, `paytype`, `paid`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Credit Card', 1)";

$params = [$amount, $firstname, $lastname, $shownname, $address, $city, $state, $zip, $phone, $email, $notes, $team, $campaignId];
...
if ($team)  {
    $query="SELECT email FROM `teams` WHERE `id`=?";
    $data=query($query,  [$team])[0];
